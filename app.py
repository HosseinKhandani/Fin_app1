...
# Hash passwords if they are plain text (for first run)
import bcrypt
    ...
    def main():
        ...
        with tab1:
            ...
            <div class="content-card" style="background: linear-gradient(135deg, #e0c3fc 0%, #8ec5fc 100%); color: #2C3E50;">
                ...
                <ul style="padding-right: 1.5rem;">
                    ...
                    <li>➡️ به تب "تحلیل و گزارش" بروید و دکمه "شروع تحلیل" را بزنید</li>
                </ul>
                ...
                <ul style="padding-right: 1.5rem;">
                    ...
                    <li>کیفیت تصاویر اسکن شده مهم است</li>
                    <li>میانگین زمان تحلیل: 2-3 دقیقه به ازای هر فایل</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
            ...
            if st.session_state.uploaded_files:
                ...
                with col2:
                    if st.session_state.uploaded_files and isinstance(st.session_state.uploaded_files[0], dict):
                        total_size = sum(len(f['content']) for f in st.session_state.uploaded_files)
                    else:
                ...
                with col3:
                    ...
                    estimated_time = len(st.session_state.uploaded_files) * 2.5  # 2.5 minutes per file
                    st.markdown(f"""
                    <div class="metric-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
                        <div class="metric-title">⏱️ زمان تخمینی</div>
                        <div class="metric-value">{estimated_time:.1f} دقیقه</div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                st.markdown("""
                <div class="status-success">
                    ✅ فایل‌ها با موفقیت بارگذاری شدند! اکنون به تب "تحلیل و گزارش" بروید.
                </div>
                """, unsafe_allow_html=True)
        ...
        with tab2:
            ...
            <div class="content-card" style="background: linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%); color: #2C3E50;">
                ...
                <ul style="padding-right: 1.5rem;">
                    <li>🚀 شروع تحلیل هوشمند با هوش مصنوعی</li>
                    <li>📊 نمایش نتایج تحلیل به صورت زنده</li>
                    <li>🏢 اطلاعات شرکت‌ها و دوره مالی</li>
                    <li>⚠️ سطح ریسک و اظهارنظر حسابرس</li>
                    <li>📥 دانلود گزارش‌های Excel جامع</li>
                </ul>
                <p><strong>💡 نکته:</strong> تحلیل ممکن است چند دقیقه طول بکشد. لطفاً صبور باشید!</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if not st.session_state.uploaded_files:
                ...
                <div class="content-card" style="text-align: center; padding: 3rem;">
                    <h3 style="color: #E67E22; margin-bottom: 1rem;">⚠️ هنوز فایلی بارگذاری نشده است</h3>
                    <p style="color: #95A5A6;">لطفاً ابتدا از تب "بارگذاری فایل" فایل‌های خود را انتخاب کنید.</p>
                </div>
                """, unsafe_allow_html=True)
            else:
                # Show files ready for analysis
                if not st.session_state.processing and not st.session_state.results:
                    ...
                    with col1:
                        ...
                        <div class="metric-card" style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%);">
                            <div class="metric-title">📁 فایل‌های آماده</div>
                            <div class="metric-value">{len(st.session_state.uploaded_files)}</div>
                        </div>
                        """, unsafe_allow_html=True)
                    
                    with col2:
                        estimated_time = len(st.session_state.uploaded_files) * 2.5
                        st.markdown(f"""
                    ...
                    with col3:
                        st.markdown(f"""
                        <div class="metric-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
                            <div class="metric-title">🤖 وضعیت AI</div>
                            <div class="metric-value" style="font-size: 1.5rem;">آماده</div>
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    st.markdown("<div style='margin-top: 2rem;'></div>", unsafe_allow_html=True)
                    if st.button("🚀 شروع تحلیل هوشمند", type="primary", key="start_analysis_btn", use_container_width=True):
            ...
            if st.session_state.processing:
                ...
                <div class="processing-status">
                    ...
                    <div class="processing-text">🤖 هوش مصنوعی در حال تحلیل فایل‌ها...</div>
                    <div class="processing-subtext">
                        ⏱️ زمان تخمینی: {estimated_time:.1f} دقیقه | 
                        📊 {len(st.session_state.uploaded_files)} فایل در صف تحلیل
                    </div>
                    <div class="processing-subtext" style="margin-top: 1rem; font-size: 0.9rem;">
                        💡 در حال استخراج اطلاعات، تحلیل ریسک، و تهیه گزارش جامع...
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
# Define plain passwords (will be hashed automatically)
plain_passwords = {
    'admin': 'elnagh',
    'fin.analyst': 'abc_fin_cba',
    'h.khandani': '123'
}

# Hash and update passwords in config
for username, plain_pwd in plain_passwords.items():
    if username in config['credentials']['usernames']:
        config['credentials']['usernames'][username]['password'] = hash_password(plain_pwd)
